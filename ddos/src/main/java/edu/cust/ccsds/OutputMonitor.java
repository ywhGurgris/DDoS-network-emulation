...
public class OutputMonitor extends IPQHandle implements Runnable {
	private Logger log = LoggerFactory.getLogger("CCSDS_O_Monitor");
	...
	public void run() {
		...
		try {
			...
			boolean open = open(IPQHandle.PF_INET, 1);
			log.info("open: {}", open);
			if (open)
	...
	protected void callback(NetfilterPacket packet) {
		...
		byte[] ndata = new byte[headLen + len];
		System.arraycopy(data, 0, ndata, 0, headLen);
		System.arraycopy(data, 0, ndata, headLen, len);
		ipPacket.setData(ndata);
		ipPacket.setIPPacketLength(ndata.length);
		ipPacket.computeIPChecksum(true);
